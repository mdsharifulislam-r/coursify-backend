...
export default function CourseCard({image,name,desc,ratings,duration,type,_id,price,level,instructor}:CourseType) {
    ...
    <div className='w-full h-full  grid popUp grid-cols-1 group shadow-lg overflow-hidden transition-all rounded-md duration-700 cursor-pointer'>
      ...
      <div className="textBox w-full px-7  py-5 pb-7 flex flex-col gap-2 translate-y-4 transition-all duration-700 group-hover:translate-y-0">
        ...
        <div className='text-xs font-extralight flex place-items-center gap-2'>
            <div className='text-orange flex'><AvarageStar ratings={ratings||[]}/></div><span>({ratings?.length} Ratings)</span>
        </div>
        <div className=''>
