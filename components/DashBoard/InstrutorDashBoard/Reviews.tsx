...
import LinkProb from './Reviews/LinkProb'

export async function ReviewUserItem({id,star,desc,date}:{id?:string,star?:string,desc?:string,date:string}){
    ...
    return (
        ...
        <td className="p-2 whitespace-nowrap">
            <div className="text-left font-light   text-slate-500">{date||new Date().toDateString()}</div>
        </td>
        <td className="p-2 whitespace-nowrap">
...
export default async function Reviews({id}:{id:string}) {
    ...
    const reviews = user?.ratings?.slice(0,4)?.reverse()?.map(data=>{
